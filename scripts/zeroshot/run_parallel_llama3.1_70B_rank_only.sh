#!/bin/bash
# Sequential zero-shot runner for LLaMA-3.1-70B-Instruct
# Rank allocation only — no PPL eval, no lm-eval benchmarks.
# Runs one experiment at a time with all visible GPUs (70B needs multi-GPU).
# Usage: bash scripts/zeroshot/run_parallel_llama3.1_70B_rank_only.sh
...
MODEL=${MODEL_PATH:-meta-llama/Llama-3.1-70B-Instruct}
RANKS="${RANKS:-128 256}"
...
METHODS="${METHODS:-care}"
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/zero-shot-rank-only}"
DYNAMIC_METHODS="no-sqrt-care care"
...
for cal in $CAL_DATASETS; do
  for method in $METHODS; do
    for rank in $RANKS; do
      # Static run
      tag="${MODEL_SHORT}-${cal}-${method}-rank${rank}"
      log_file="${LOG_DIR}/${tag}.log"
      echo "[$(date +%H:%M:%S)] Running: $method rank=$rank cal=$cal"
      ...
      # Dynamic run (only for care/no-sqrt-care)
      if echo "$DYNAMIC_METHODS" | grep -qw "$method"; then
