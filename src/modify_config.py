...
settings["qwen3"] = settings["llama"]
settings["mistral"] = settings["llama"]
settings["mimo"] = settings["llama"]

_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
_TRANSFORMERS_ROOT = os.path.join(_THIS_DIR, "transformers")

transformers_dirs = {
    "llama": os.path.join(_TRANSFORMERS_ROOT, "llama"),
    "gemma2": os.path.join(_TRANSFORMERS_ROOT, "gemma2"),
    "mixtral": os.path.join(_TRANSFORMERS_ROOT, "mixtral"),
    "deepseek_v3": os.path.join(_TRANSFORMERS_ROOT, "deepseek_v3"),
}
transformers_dirs["qwen2"] = transformers_dirs["llama"]
transformers_dirs["qwen3"] = transformers_dirs["llama"]
transformers_dirs["mistral"] = transformers_dirs["llama"]
mla_dir = os.path.join(_TRANSFORMERS_ROOT, "mla.py")
...
def modify_config(model, config_path: str, args):
    ...
    config["attention_bias"] = getattr(model.model.layers[0].self_attn, "attention_bias", False)
    ...
    if not os.path.isdir(transformers_dir):
        raise FileNotFoundError(
            f"Transformers template directory not found: {transformers_dir}"
        )
    ...
    if not os.path.isfile(mla_dir):
        raise FileNotFoundError(f"MLA template file not found: {mla_dir}")
