...
class PartialRope(nn.Module):
    ...
    def joint_complex_pca(self, Z: list[torch.Tensor], freqfold: int = 1) -> torch.Tensor:
        ...
        eigen_vecs = []
        num_freq_blocks = self.head_dim // 2 // freqfold
        desc = f"Layer {self.layer_idx} partial-rope PCA"
        for i in tqdm(range(num_freq_blocks), desc=desc, leave=False):
            covariance = None
            for Z_batch in Z:
                b,n,d = Z_batch.shape
                ...
                head_batch_i = torch.sum(head_batch_i.mT @ head_batch_i, dim=0)
                covariance = head_batch_i if covariance is None else covariance + head_batch_i
            damp = 0.01 * torch.mean(torch.diag(covariance))
            diag = torch.arange(covariance.shape[-1]).to(self.k_proj.weight.device)
            covariance[diag, diag] = covariance[diag, diag] + damp
            X_eig = torch.linalg.eigh(covariance)
            del covariance
...
def partial_rope(model, tokenizer, train_loader, test_loader, **kwargs):
    ...
    def partial_rope_freqfold(model, ori_qkv_outputs, test_loader, freqfold: int, collapse):
        desc = f"Building PartialRoPE layers (freqfold={freqfold})"
        for layer_idx, layer in enumerate(tqdm(model.model.layers, desc=desc)):
            ...
            print(f"[PartialRoPE] Layer {layer_idx} built in {layer_elapsed:.1f}s")
    ...
    if freqfold != "auto":
        ...
        head_dim = getattr(model_original.config, "head_dim", None)
        if head_dim is None:
            head_dim = model_original.config.hidden_size // model_original.config.num_attention_heads
            model_original.config.head_dim = head_dim

        print("Auto freqfold detection...")

        best_freqfold = freqfold = collapse
        best_ppl = float("inf")
        while freqfold <= head_dim // 2:
